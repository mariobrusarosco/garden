...
option = CreditCardInstallmentOption(**data)  # ✅ Success
```
...
invoice_data = {
    "installment_options": [ai_response]  # ❌ Pydantic validation error!
}
...
invoice_data = {
    "installment_options": [ai_response.model_dump()]  # ✅ Convert to dict
}

invoice = InvoiceIn(**invoice_data)  # ✅ Success
```
...
financial_data.installment_options = [
    InstallmentOption(months=10, total="R$ 960,60"),  # ❌ Wrong class
    InstallmentOption(months=4, total="R$ 766,12"),  # ❌ Wrong class
]
```
...
class CreditCardRawInvoice(BaseModel):
    installment_options: List[CreditCardInstallmentOption]  # ✅ Expected class
```
...
],  # ✅ Now it's [{"months": 10, "total": "R$ 960,60"}]
```
...
user = User(**data)  # ✅ Pydantic creates object from dict
```
...
data = user.model_dump()  # ✅ {"name": "John", "age": 30}
```
...
user2 = User.model_validate(user1)  # ✅ Same class, works
```
...
user = User.model_validate(person)  # ❌ Fails even if same fields
```
