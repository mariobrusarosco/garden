...
2. Text → OpenAI API with TransactionData schema
3. OpenAI extracts movement_type correctly ✅
4. Returns List[TransactionData] with all fields ✅
5. Direct assignment to RawBankStatement ✅
...
```python
# ✅ Good: Single authoritative schema
app/domains/transactions/schemas.py:
  - TransactionData (for AI parsing)
  - TransactionBase (for domain operations)

# ❌ Avoid: Duplicate schemas in different layers
...
# ❌ Avoid: Domain depends on infrastructure
...
```python
# ✅ Good: Direct usage of shared schemas
"transactions": financial_data.transactions

# ❌ Avoid: Manual conversion between identical structures
