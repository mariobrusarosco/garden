...
For a _development environment_ we can use `ts-node-dev` (with alias path convertion: 'ts-config-paths')
...
We use the "mode" prop and create a .env.[mode].local to also load an extra _.env_ file
